    ...
    <table class="datatable display" style="width:100%">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $so->notes ?? '-' }}</td>
                <td>
                    <span class="badge {{ $so->ppic_status }}">{{ $so->ppic_status == 'rejected' ? 'Reschedule' : ucfirst($so->ppic_status) }}</span>
                </td>
                <td>
                    @if(Auth::user()->role == 'ppic' && $so->ppic_status == 'pending')
                        ...
                        <button class="btn btn-danger" onclick="openRejectModal({{ $so->id }})">Reschedule</button>
                    @endif
                </td>
            </tr>
            @endforeach
